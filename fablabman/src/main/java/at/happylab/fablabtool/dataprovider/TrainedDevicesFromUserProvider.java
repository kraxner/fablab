...
public class TrainedDevicesFromUserProvider extends SortableDataProvider<Device> implements
		Serializable {
	...
	public TrainedDevicesFromUserProvider(User user) {
		setSort("name", true);
	...
	public Iterator<Device> iterator(int first, int count) {
		List<Device> results = this.user.getTrainedForDevices();
	...
	public int size() {
		return this.user.getTrainedForDevices().size();
	}
